//
//  ContentView.swift
//  MyWaterApp Watch App
//
//  Created by Hussain Almousawi on 1/29/23.
//
    ...
    var body: some View {
        ...
        .alert(isPresented: $isAlertPresented, content: {
            Alert(title: Text("Are you sure?"), primaryButton: .default(Text("Yes"), action: {
                total = 0
                data.session.transferUserInfo(["WatchCount":total])
        ...
        .sheet(isPresented: $isPresented) {
            ...
            VStack {
                ...
                DigitTextView(placeholder: "Enter...", text: $water, presentingModal: false)

                Button(action: {
                    if var intWater = Int(water) {
                        intWater += 330
                        water = "\(intWater)"
                    } else {
                        water = "330"
...
public struct DigitTextView: View {
...
struct Content_View_Previews: PreviewProvider {
    static var previews: some View{
        ScrollView {
            ForEach(0 ..< 4) { item in
                DigitTextView(placeholder: "Placeholder", text: .constant(""), presentingModal: false, alignment: .leading)
            }
